...
import { DataStoreService, StoredStudent, StoredTeacher } from './data-store.service';
...
export class ConnectionsComponent implements OnInit {
  ...
  constructor(private store: DataStoreService, public connections: ConnectionService) {}
  ...
  isTeacherAssigned(teacherId: number): boolean {
    ...
    return this.connections.getConnectionsByStudent(studentId)
  ...
  toggleAssignment(teacherId: number, checked: boolean) {
    ...
    if (checked) {
      ...
      const existing = this.connections.getConnectionsByStudent(studentId)
