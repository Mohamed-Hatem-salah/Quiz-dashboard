...
import { DataStoreService, StoredStudent, StoredTeacher } from '../connections/data-store.service';
...
@Component({
  ...
  styleUrls: ['./relashion-map.component.css'] // <-- small fix: should be style**Urls**
})
export class RelashionMapComponent implements OnInit {
  ...
  readonly teacherWithAssignments = computed(() => {
    ...
    return this.teachers
      .filter(t => !term || t.name.toLowerCase().includes(term) || t.subject.toLowerCase().includes(term))
      .map(t => {
